...
RuXi = zeros(wvt.Nz,nUnknown);
RvXi = zeros(wvt.Nz,nUnknown);
RetaXi = zeros(wvt.Nz,nUnknown);
...
Rv(:,ib) = vBottom;
Reta(:,ib) = etaBottom;
RuXi(:,ib) = -1i*l*profile.psiXi;
RvXi(:,ib) = 1i*k*profile.psiXi;
RetaXi(:,ib) = profile.etaXi;
Rp(:,ib) = pBottom;
Rp(:,ip) = F;
RpXi(:,ib) = pBottomXi;
RpXi(:,ip) = Fxi;
...
block.reconstruction = struct("uHat",Ru,"vHat",Rv,"wHat",Rw,"etaHat",Reta, ...
    "uHatXi",RuXi,"vHatXi",RvXi,"etaHatXi",RetaXi,"pressure",Rp,"pressureXi",RpXi);
