...
> **Milestone-10.2.1 scientific checkpoint:** the exact coupled Sylvester correction recovers the fixed-discretization zonal production-wave projector in one update, but that physical projector drifts by \(5.96\times10^{-4}\) across primitive degrees \(8,10,12\). The measured outcome is `coupled-block-isolation-blocker`. Per the approved stopping rule, no iterative Krylov/Jacobi--Davidson stage or Milestone 10.3 work was attempted. Read [Read me first: terrain-energy Galerkin status](READ_ME_FIRST.md) and the [Milestone 10.2.1 result](milestones.md#milestone-1021-coupled-block-internal-wave-correction) before continuing. The implemented mean-depth generator and scattering classes remain a validated first-order baseline.
