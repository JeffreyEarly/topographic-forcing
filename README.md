...
> **Next approved experiment:** [Milestone 10.2.6](milestones.md#milestone-1026-complete-wkb-chebyshev-primitive-oracle) compares a complete WKB-stretched Chebyshev primitive state with the validated Legendre oracle. The prior `analytic-modal-seed` result showed that exact mixed-boundary modes improve the stationary projector but do not provide economical complete coverage. The roadmap therefore makes compatible completeness—not factor-two modal compression—the prerequisite for a forward model. Read [Read me first: terrain-energy Galerkin status](READ_ME_FIRST.md) before continuing. The implemented mean-depth generator and scattering classes remain a validated first-order baseline.
