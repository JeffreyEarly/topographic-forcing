...
> **Scientific checkpoint before Milestone 10.2.5:** the fixed-$\kappa$ wave coordinates remain verified, but trained signed-Robin geostrophic coordinates with global stationary $G_1$ dressing return `dressed-stationary-blocker`. The next approved experiment reverses the construction order: it forms the global terrain-tangent bottom-trace space first and compresses only its homogeneous vertical interior. Milestone 10.3 remains inactive unless this [global tangent-scalar stationary compression oracle](milestones.md#milestone-1025-global-tangent-scalar-stationary-compression-oracle) passes every physical gate with factor-two compression. Read [Read me first: terrain-energy Galerkin status](READ_ME_FIRST.md) before continuing. The implemented mean-depth generator and scattering classes remain a validated first-order baseline.
...
Milestone 10.2.5 is specified but not yet implemented. It will construct the conjugate-closed kernel of the projected bottom-trace tangency operator, factor the polynomial stationary oracle into homogeneous-interior and tangent-trace lift sectors, and only then test mixed-boundary modal compression. Polynomial oracle vectors remain validation data rather than candidate coordinates. Only `global-tangent-acceleration` authorizes Milestone 10.3; see the [full milestone specification](milestones.md#milestone-1025-global-tangent-scalar-stationary-compression-oracle).
