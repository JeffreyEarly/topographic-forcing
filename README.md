...
> **Next approved increment:** [Milestone 10.3](milestones.md#milestone-103-complete-spectral-production-state-gate) builds the complete dense production state in the selected Legendre representation. [Milestone 10.2.6](milestones.md#milestone-1026-complete-wkb-chebyshev-primitive-oracle) completed as `wkb-spectral-fallback`: constant-$N$ WKB and Legendre projectors agree to $5.27\times10^{-14}$, but the variable-$N$ WKB bottom defect stagnates near $1.5\times10^{-5}$ while the Legendre outer-support APV and bottom defects close at $1.13\times10^{-13}$ and $4.28\times10^{-12}$. Read [Read me first: terrain-energy Galerkin status](READ_ME_FIRST.md) before continuing. The implemented mean-depth generator and scattering classes remain a validated first-order baseline.
...
Milestone 10.2.4 is exposed by `auditTerrainDressedRobinStationaryAmbient`. It trains the signed Robin length on the stationary sector alone. The global $G_1$ correction is applied only to scalar combinations satisfying bottom tangency; complementary non-tangent Robin directions remain admissible flat coordinates and are retained for later dynamical classification.
...
No candidate passes the prerequisite provider, centered-inclusion, conjugacy, APV Green-identity, and stationary-compression gates, so the independent finite-amplitude validation is not run and no $\ell_b$ is frozen. Among the unqualified candidates, $\ell_b/D=-1$ gives the smallest stationary-projector defect, $1.20\times10^{-2}$, compared with $1.26\times10^{-2}$ for the ordinary boundary. Its Gram condition number is $1.33\times10^{15}$, compared with $1.99\times10^7$ for the ordinary family. The result is `dressed-stationary-blocker`, leaving Milestone 10.3 inactive.
...
Milestone 10.2.5 is exposed by [`auditGlobalTangentStationaryCompression`](<@WVTerrainEnergyGalerkin/auditGlobalTangentStationaryCompression.m>) and tested by [`TestWVTerrainEnergyGlobalTangentStationaryCompression`](UnitTests/TestWVTerrainEnergyGlobalTangentStationaryCompression.m). The exact polynomial factorization has maximum projector defect $1.51\times10^{-14}$ with no dimension loss, and the rank-revealing physical-energy lift solve closes at roundoff. The isolated provider pinned to `df86687` fails its order-128/256 convergence gate, returning `global-tangent-provider-blocker` before a modal count is frozen. This is a provider limitation, not evidence against the exact factorization.

[Milestone 10.2.5.1](milestones.md#milestone-10251-analytic-mixed-boundary-attribution-control) is exposed by [`auditAnalyticGlobalTangentStationaryCompression`](<@WVTerrainEnergyGalerkin/auditAnalyticGlobalTangentStationaryCompression.m>) and tested by [`TestWVTerrainEnergyAnalyticGlobalTangentStationaryCompression`](UnitTests/TestWVTerrainEnergyAnalyticGlobalTangentStationaryCompression.m). It returns `analytic-modal-seed`: exact analytic controls pass to $5.86\times10^{-15}$ and the propagated order-128 provider difference is $6.80\times10^{-11}$, but the best training stationary-projector defect is $1.55\times10^{-2}$ and the internal-wave projector defect is $6.99\times10^{-1}$. The attribution control therefore rules out provider conditioning as the cause of the physical failure; it does not authorize variable-stratification qualification or Milestone 10.3.

[Milestone 10.2.6](milestones.md#milestone-1026-complete-wkb-chebyshev-primitive-oracle) is exposed by [`auditWKBPrimitiveForwardState`](<@WVTerrainEnergyGalerkin/auditWKBPrimitiveForwardState.m>) and tested by [`TestWVTerrainEnergyWKBPrimitiveForwardState`](UnitTests/TestWVTerrainEnergyWKBPrimitiveForwardState.m). It returns `wkb-spectral-fallback`. The WKB and Legendre constant-$N$ projectors agree to $5.27\times10^{-14}$, but for exponential stratification the WKB bottom defect remains $1.35$--$1.51\times10^{-5}$ over degrees 8--16 even as the projector converges to $3.19\times10^{-13}$. The complete Legendre state passes at degree 20: outer-support APV and bottom defects are $1.13\times10^{-13}$ and $4.28\times10^{-12}$, padding drift is $9.41\times10^{-14}$, and the two-dimensional control passes. Milestone 10.3 is therefore authorized with Legendre as the complete production coordinate.
