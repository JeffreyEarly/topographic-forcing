...
**This branch has completed Milestone 10 with classification `residual-enrichment-acceleration`.** Milestones 7 and 8 provide a passing finite-amplitude primitive weak oracle and a tangency-defined stationary balanced sector. Milestones 9 and 9.1 preserve that sector, construct the complete physical-energy eigensystem, and identify a converged stationary projector and one conjugate-closed internal-wave projector while retaining every unvalidated direction in an explicit unresolved projector. Milestones 9.2 and 9.3 show why modifying separate vertical problems or local bottom endpoints is insufficient: the leading terrain correction is a global horizontal convolution. Milestone 9.4 constructs that correction from the verified \(H_1,J_1,R_1,G_1\) derivatives. Milestone 10 then applies repeated exact finite-amplitude residual corrections without changing \(H_\gamma\), \(J_\gamma\), the stationary space, or the primitive ambient eigensystem. Three corrections reproduce the independent dense internal-wave block with compression factor `6.22`. Active bottom directions remain in the unresolved complement pending the complete topographic-boundary continuation gates. Milestone 11 has not begun.
...
- starting Milestone 11 or later work without using the Milestone-10 enriched blocks in the primitive ambient representation and preserving the Milestone-9.1 stationary, validated-wave, and unresolved projectors;
...
Milestones 7–10 supply the finite-amplitude dense weak oracle, trusted stationary balanced space, complete dense eigensystem, one validated internal-wave projector, a quantified unresolved remainder, controlled rejections of both flat-Dirichlet and slope-compatible per-wavenumber compression, a verified global first-order dressed seed, and a converged exact-residual enrichment of the internal-wave block. The primitive polynomial representation remains the ambient oracle. The complete unresolved algebraic space must remain available during any matrix-free implementation.
...
The focused Milestone-10 suite passes 9 tests, the complete repository suite passes 190 tests, and `checkcode` reports no issues in all 92 MATLAB files.
