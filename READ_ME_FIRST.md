...
**This branch is paused after Milestone 10.2.2 with outcome `cascade-slow`.** The eight-dimensional zonal production-wave block is physically accurate and separated from its complement by \(5.16\times10^{11}\) times backward uncertainty. Its horizontal geometric-scattering tail falls below \(10^{-8}\) by order four, but the vertical primitive tail remains \(6.35\times10^{-7}\) at degree fourteen. Milestone 10.3, matrix-free construction, and time integration remain inactive until the vertical tail is resolved economically.

**The production contract contains only physically declared coordinates.** It uses native WaveVortexModel fixed-\(\kappa\) wave and inertial modes, APV-bearing balanced modes, one existing complete zero-APV bottom inversion per retained nonzero horizontal wavenumber, and compatible MDA and mean-bottom coordinates at \(\kappa=0\). Every coordinate has a physical family, units, native index where applicable, Fourier partner, and converged primitive-oracle image. Guard modes remain evaluation infrastructure, omitted primitive directions are truncation diagnostics, and no unresolved coordinate enters the production layout. A future reformulation must establish a vertically stable complete internal-wave projector before Milestones 10.3–10.4 can assign every production coordinate to the finite-terrain stationary, internal-wave, or topographic-wave projector.
...
The exact correction passes its equation, orthogonality, fixed-degree physical, and padding gates, but the nested vertical projector does not converge. Milestone 10.2.2 isolates the cause: horizontal tail amplitudes decrease from \(7.58\times10^{-4}\) at scattering order one to \(4.66\times10^{-9}\) at order four, while the degree-\(8,10,12,14\) vertical tails are \(2.77\times10^{-4}\), \(3.72\times10^{-5}\), \(5.03\times10^{-6}\), and \(6.35\times10^{-7}\). The block remains eight dimensional, its Ritz, APV, bottom, and strong defects are \(2.74\times10^{-15}\), \(1.18\times10^{-12}\), \(1.01\times10^{-13}\), and \(1.16\times10^{-7}\), and no resonant enlargement occurs. The result is slow vertical convergence rather than a horizontal-cascade or spectral-isolation failure. Milestone 10.3 must not begin until a vertically adapted representation or higher-degree oracle reaches the \(10^{-8}\) projector gate economically. Guard modes remain construction infrastructure, while primitive directions omitted from the declared state remain visible through tail and unresolved-energy diagnostics.

The public entry point is `auditGeometricCascadeIsolation`. Its measured outcome is `cascade-slow`; the other declared outcomes remain `cascade-resolved`, `resonant-block-required`, and `cascade-nonconvergent`. The optional content-addressed cache writes only beneath ignored `output/`, validates every signature before reuse, and is disabled without disk writes by default. Only `cascade-resolved` authorizes bottom/topographic-wave classification.
