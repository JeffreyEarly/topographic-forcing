...
**This branch is paused before Milestone 10.2.2, the geometric-cascade and spectral-isolation audit.** Milestone 10.2.1 established that the exact coupled Sylvester correction repairs the frequency-local algorithm at each fixed discretization: one update recovers the degree-12 zonal production-wave projector with Ritz residual \(1.69\times10^{-12}\), dense-projector defect \(4.03\times10^{-10}\), APV defect \(1.16\times10^{-9}\), and bottom defect \(3.50\times10^{-13}\). The same projector changes by \(5.96\times10^{-4}\) across primitive degrees \(8,10,12\), far above the \(10^{-8}\) isolation gate. Milestone 10.2.2 will determine whether this drift is an unresolved geometric-scattering tail or a missing resonant block. Milestone 10.3, matrix-free construction, and time integration remain inactive.
...
The exact correction passes its equation, orthogonality, fixed-degree physical, and padding gates, but the nested vertical projector does not converge. This establishes the approved isolation stop before the iterative method or two-dimensional control. Milestone 10.2.2 now defines the required experiment: continue the physical projector from flat terrain, vary horizontal scattering order and vertical degree independently, measure its omitted physical-energy tail, and enlarge the tracked block only when backward-error separation is lost. Milestone 10.3 must not begin until this gate establishes a stable and economical complete internal-wave sector. Guard modes remain construction infrastructure, while primitive directions omitted from the declared state remain visible through oracle reconstruction errors and unresolved-energy diagnostics.

The planned public entry point is `auditGeometricCascadeIsolation`. Its primary outcomes are `cascade-resolved`, `cascade-slow`, `resonant-block-required`, and `cascade-nonconvergent`. Only `cascade-resolved` authorizes bottom/topographic-wave classification.
