...
**This branch is paused before Milestone 10.2.6, the complete WKB--Chebyshev primitive oracle.** Milestone 10.2.5.1 ended as `analytic-modal-seed`: the exact constant-$N$ mixed-boundary family passes its analytic controls to $5.86\times10^{-15}$ and its propagated stationary projector agrees with the pinned order-128 provider to $6.80\times10^{-11}$, but its final $1.55\times10^{-2}$ stationary error and the wave and bottom defects remain far above tolerance. Provider conditioning is not the limiting physical error, and economical modal compression is no longer a prerequisite for forward evolution.

**The forward target is now a complete compatible spectral state.** Milestone 10.2.6 compares a WKB-stretched Chebyshev representation with the mature Legendre primitive representation while retaining every admissible velocity, displacement, bottom, MDA, and constraint-compatible direction. Passing physical energy, projected APV, bottom evolution, reconstruction, and convergence is mandatory. Factor-two compression determines whether WKB is an acceleration, not whether a complete forward model may proceed. Internal-wave, geostrophic, and topographic-wave families remain valuable for initialization, diagnostics, and preconditioning; unclassified directions are retained rather than deleted from the complete production state.
