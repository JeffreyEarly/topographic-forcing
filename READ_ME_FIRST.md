...
**This branch is paused after Milestone 10.2.3 with outcome `geostrophic-modal-blocker`.** The fixed-\(\kappa\) wave coordinates and isolated one-dimensional EVP provider pass their flat controls. The ordinary flat APV-bearing geostrophic modes and explicit zero-APV bottom inversion converge systematically toward the exact finite-terrain stationary space, but at the largest tested count the stationary representation defect remains \(4.26\times10^{-3}\), the internal-wave projector defect is \(2.33\times10^{-4}\), and compression has fallen to \(1.68\). Milestone 10.3, matrix-free construction, and time integration remain inactive.
...
The pre-Milestone-6.7 checkpoint is commit [`d5ab18f`](https://github.com/JeffreyEarly/topographic-forcing/tree/d5ab18f) on the [`terrain-energy-galerkin`](https://github.com/JeffreyEarly/topographic-forcing/tree/terrain-energy-galerkin) branch. The current branch contains the completed projected, tangent boundary-complete, finite-amplitude weak, physical-subspace, and modal-ambient oracles. The matching mathematics is maintained in `finite-terrain-projection-problem.tex` and `terrain-energy-galerkin.tex` in the `ape-apv-bottom-topography` literature repository.
...
The latest public entry point is `auditWaveVortexModalAmbient`. Its measured outcome is `geostrophic-modal-blocker`; the fixed-\(\kappa\) wave/provider controls pass, while the ordinary flat stationary coordinates fail the physical and economy gates. The optional content-addressed cache writes only beneath ignored `output/`, validates every signature before reuse, and is disabled without disk writes by default. Only `wave-vortex-modal-acceleration` would authorize bottom/topographic-wave classification.
