...
**This branch is paused after Milestone 10.2.6 and before Milestone 10.3, the complete spectral production-state gate.** Milestone 10.2.6 completed as `wkb-spectral-fallback`. Constant-$N$ WKB--Chebyshev and Legendre projectors agree to $5.27\times10^{-14}$, but the variable-stratification WKB bottom defect remains near $1.5\times10^{-5}$ under vertical refinement. The complete Legendre state passes the physical and convergence gates and is the selected forward coordinate.

**The forward target is a complete compatible Legendre spectral state.** At degree 20 and outer support `[1 2]`, the Milestone-10.2.6 APV and bottom defects are $1.13\times10^{-13}$ and $4.28\times10^{-12}$, padding-two/three projector drift is $9.41\times10^{-14}$, and the independent two-dimensional control passes. Milestone 10.3 must now freeze this complete representation and verify arbitrary-state reconstruction and complementary stationary/dynamical projectors before time integration. Internal-wave, geostrophic, and topographic-wave families remain valuable for initialization, diagnostics, and preconditioning; no admissible direction is deleted.

The pre-Milestone-6.7 checkpoint is commit [`d5ab18f`](https://github.com/JeffreyEarly/topographic-forcing/tree/d5ab18f) on the [`terrain-energy-galerkin`](https://github.com/JeffreyEarly/topographic-forcing/tree/terrain-energy-galerkin) branch. The current branch contains the completed projected, tangent boundary-complete, finite-amplitude weak, physical-subspace, modal-ambient, terrain-dressed, and global tangent-scalar oracles. The matching mathematics is maintained in `finite-terrain-projection-problem.tex` and `terrain-energy-galerkin.tex` in the `ape-apv-bottom-topography` literature repository.
...
Milestones 7–10.1 supply the finite-amplitude dense weak oracle, trusted stationary balanced space, complete dense eigensystem, one validated internal-wave projector, controlled rejections of both flat-Dirichlet and slope-compatible per-wavenumber compression, a verified global first-order dressed seed, a converged exact-residual enrichment of the internal-wave block, and an explicit production coordinate contract. The primitive polynomial representation remains the ambient oracle during Milestones 10.2–10.4, including the intervening Milestones 10.2.1 and 10.2.2, so that omitted directions and truncation errors can be measured independently. It is not the intended online state, and its unresolved algebraic completion will not be carried into matrix-free evolution.

Milestone 10.2.1 tested that reformulation. With the complete Ritz basis \(Y\), residual \(R=iJ_\gamma Y-H_\gamma Y\Theta\), and \(U=[G_\gamma\;Y]\), it solved the exact constrained coupled correction
...
The exact correction passes its equation, orthogonality, fixed-degree physical, and padding gates, but the nested vertical projector does not converge. Milestone 10.2.2 isolates the cause: horizontal tail amplitudes decrease from \(7.58\times10^{-4}\) at scattering order one to \(4.66\times10^{-9}\) at order four, while the degree-\(8,10,12,14\) vertical tails are \(2.77\times10^{-4}\), \(3.72\times10^{-5}\), \(5.03\times10^{-6}\), and \(6.35\times10^{-7}\). The block remains eight dimensional, its Ritz, APV, bottom, and strong defects are \(2.74\times10^{-15}\), \(1.18\times10^{-12}\), \(1.01\times10^{-13}\), and \(1.16\times10^{-7}\), and no resonant enlargement occurs. The result is slow vertical convergence rather than a horizontal-cascade or spectral-isolation failure. Milestone 10.2.3 then replaces the candidate polynomial ambient by flat physical modes. It confirms that the fixed-\(\kappa\) wave sector is accurate, but finds that ordinary flat geostrophic modes plus the explicit bottom inversion converge too slowly to the exact terrain stationary inclusion. At modal count twelve the wave-projector defect is \(2.33\times10^{-4}\), the stationary representation defect is \(4.26\times10^{-3}\), and the compression factor is only \(1.68\). Guard modes remain construction infrastructure, while primitive directions omitted from the declared state remain visible through projector and unresolved-energy diagnostics.

Milestone 10.2.4 tests whether signed-Robin vertical localization plus the verified global stationary derivative $G_1$ repairs that balanced-coordinate tail. Only bottom-tangent scalar combinations receive $G_1$; non-tangent Robin directions remain admissible flat coordinates, so the full candidate dimension is preserved. At geostrophic count eight, the best unqualified finite choice $\ell_b/D=-1$ has stationary-projector defect $1.20\times10^{-2}$, compared with $1.26\times10^{-2}$ for the ordinary boundary, while its Gram condition number increases from $1.99\times10^7$ to $1.33\times10^{15}$. Dressing reduces stationary weak-row and bottom defects by factors of a few but does not materially improve the projector. No candidate passes the prerequisite training gates, so no Robin length is frozen and finite-amplitude validation is skipped.

Milestone 10.2.5 remains the historical `global-tangent-provider-blocker`: the polynomial factorization and physical-energy-whitened block lift pass, while the pinned provider misses its order-128/256 qualification. Milestone 10.2.5.1 isolates that provider error from the physical span by using the exact constant-$N$ family. The maximum analytic-versus-provider vertical and propagated projector differences are $9.32\times10^{-12}$ and $6.80\times10^{-11}$, both negligible compared with the measured physical defects.

[Milestone 10.2.5](milestones.md#milestone-1025-global-tangent-scalar-stationary-compression-oracle) is complete and preserved as provider evidence. [Milestone 10.2.5.1](milestones.md#milestone-10251-analytic-mixed-boundary-attribution-control) is also complete with `analytic-modal-seed`. Its stationary-projector defects decrease from $4.79\times10^{-1}$ to $1.55\times10^{-2}$ over counts two through twelve, but its best internal-wave projector, APV, bottom, and strong defects are $6.99\times10^{-1}$, $2.50\times10^{-4}$, $1.66\times10^{-1}$, and $6.90\times10^{-2}$. The exact family is a useful convergence seed, not a production basis. That outcome did not authorize variable-$N$ provider work or Milestone 10.3; the subsequent complete Legendre result in Milestone 10.2.6 now authorizes Milestone 10.3 by a different route.

[Milestone 10.2.6](milestones.md#milestone-1026-complete-wkb-chebyshev-primitive-oracle) is complete with `wkb-spectral-fallback`. The complete WKB proposal preserves the coordinate map, quadrature, energy, APV, and constant-$N$ equivalence controls, but for variable $N$ its finite spaces do not preserve the physical derivative inclusion because $\partial_\xi=s_0'(\xi)\partial_{s_0}$ has nonconstant $s_0'$. The persistent bottom residual is therefore a compatibility failure of this particular unweighted Chebyshev sequence, not a failure of complete spectral coordinates generally. The mature physical-coordinate Legendre sequence passes and authorizes Milestone 10.3. Variable-$N$ provider tuning and aggressive modal compression remain deferred optimizations.
...
The coupled-block oracle is exposed by [`auditCoupledBlockInternalWaveCorrection`](<@WVTerrainEnergyGalerkin/auditCoupledBlockInternalWaveCorrection.m>), tested by [`TestWVTerrainEnergyCoupledBlockInternalWaveCorrection`](UnitTests/TestWVTerrainEnergyCoupledBlockInternalWaveCorrection.m), and recorded in [Milestone 10.2.1](milestones.md#milestone-1021-coupled-block-internal-wave-correction). It returns `coupled-block-isolation-blocker` and leaves `iterative.wasAttempted=false`, as required by the stopping rule.
...
The geometric-cascade oracle is exposed by [`auditGeometricCascadeIsolation`](<@WVTerrainEnergyGalerkin/auditGeometricCascadeIsolation.m>), tested by [`TestWVTerrainEnergyGeometricCascadeIsolation`](UnitTests/TestWVTerrainEnergyGeometricCascadeIsolation.m), and recorded in [Milestone 10.2.2](milestones.md#milestone-1022-geometric-cascade-and-spectral-isolation-audit). It returns `cascade-slow`, with a resolved horizontal cascade and a slowly converging vertical primitive tail.
...
The flat wave–vortex modal ambient oracle is exposed by [`auditWaveVortexModalAmbient`](<@WVTerrainEnergyGalerkin/auditWaveVortexModalAmbient.m>), tested by [`TestWVTerrainEnergyWaveVortexModalAmbient`](UnitTests/TestWVTerrainEnergyWaveVortexModalAmbient.m), and recorded in [Milestone 10.2.3](milestones.md#milestone-1023-flat-wavevortex-modal-ambient-oracle). It returns `geostrophic-modal-blocker`: the wave/provider controls pass, but ordinary flat geostrophic coordinates do not represent the exact terrain stationary inclusion with the required accuracy and economy.
...
The terrain-dressed stationary-coordinate oracle is exposed by [`auditTerrainDressedRobinStationaryAmbient`](<@WVTerrainEnergyGalerkin/auditTerrainDressedRobinStationaryAmbient.m>), tested by [`TestWVTerrainEnergyTerrainDressedRobinStationaryAmbient`](UnitTests/TestWVTerrainEnergyTerrainDressedRobinStationaryAmbient.m), and recorded in [Milestone 10.2.4](milestones.md#milestone-1024-terrain-dressed-robin-stationary-coordinate-oracle). It returns `dressed-stationary-blocker` before independent finite-amplitude validation because no signed-Robin training candidate passes the complete stationary gate.
...
The global tangent-scalar oracle is exposed by [`auditGlobalTangentStationaryCompression`](<@WVTerrainEnergyGalerkin/auditGlobalTangentStationaryCompression.m>), tested by [`TestWVTerrainEnergyGlobalTangentStationaryCompression`](UnitTests/TestWVTerrainEnergyGlobalTangentStationaryCompression.m), and recorded in [Milestone 10.2.5](milestones.md#milestone-1025-global-tangent-scalar-stationary-compression-oracle). It returns `global-tangent-provider-blocker` after proving the exact polynomial factorization and whitened lift construction.

The analytic attribution oracle is exposed by [`auditAnalyticGlobalTangentStationaryCompression`](<@WVTerrainEnergyGalerkin/auditAnalyticGlobalTangentStationaryCompression.m>), tested by [`TestWVTerrainEnergyAnalyticGlobalTangentStationaryCompression`](UnitTests/TestWVTerrainEnergyAnalyticGlobalTangentStationaryCompression.m), and recorded in [Milestone 10.2.5.1](milestones.md#milestone-10251-analytic-mixed-boundary-attribution-control). It returns `analytic-modal-seed` and stops before variable-stratification solver work or bottom/topographic-wave classification.
