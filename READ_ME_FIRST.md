...
**This branch has completed Milestone 10.2.1 with outcome `coupled-block-isolation-blocker`.** The exact coupled Sylvester correction repairs the frequency-local correction algorithm at each fixed discretization: one update recovers the degree-12 zonal production-wave projector with Ritz residual \(1.69\times10^{-12}\), dense-projector defect \(4.03\times10^{-10}\), APV defect \(1.16\times10^{-9}\), and bottom defect \(3.50\times10^{-13}\). The same physical projector changes by \(5.96\times10^{-4}\) across primitive degrees \(8,10,12\), far above the \(10^{-8}\) isolation gate. The iterative stage was therefore not authorized, and Milestone 10.3, matrix-free construction, and time integration remain inactive.
...
The exact correction passes its equation, orthogonality, fixed-degree physical, and padding gates, but the nested vertical projector does not converge. This establishes the approved isolation stop before the iterative method or two-dimensional control. A future increment must determine why the primitive internal-wave projector changes under vertical refinement without hiding that drift through dense construction vectors, a frequency cutoff, or mode deletion. Milestone 10.3 must not begin until a new gate establishes a stable complete internal-wave sector. Guard modes remain construction infrastructure, while primitive directions omitted from the declared state remain visible through oracle reconstruction errors and unresolved-energy diagnostics.
