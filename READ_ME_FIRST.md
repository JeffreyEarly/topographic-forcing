...
**This branch is paused before Milestone 10.2.5, following the Milestone-10.2.4 outcome `dressed-stationary-blocker`.** The fixed-$\kappa$ wave coordinates and isolated one-dimensional EVP provider pass their flat controls. Training signed-Robin geostrophic functions and applying the global stationary $G_1$ correction to their bottom-tangent combinations improves weak-row and bottom residuals, but the best stationary-projector defect remains $1.20\times10^{-2}$. The next approved experiment constructs the global terrain-tangent bottom-trace kernel first and uses mixed-boundary modes only to compress its homogeneous vertical interior. Milestone 10.3, matrix-free construction, and time integration remain inactive unless that experiment returns `global-tangent-acceleration`.

**The production contract contains only physically declared coordinates.** It uses native WaveVortexModel fixed-\(\kappa\) wave and inertial modes, APV-bearing balanced modes, one existing complete zero-APV bottom inversion per retained nonzero horizontal wavenumber, and compatible MDA and mean-bottom coordinates at \(\kappa=0\). Every coordinate has a physical family, units, native index where applicable, Fourier partner, and converged primitive-oracle image. Guard modes remain evaluation infrastructure, omitted primitive directions are truncation diagnostics, and no unresolved coordinate enters the production layout. A future reformulation must retain the verified fixed-\(\kappa\) wave sector while constructing a terrain-adapted stationary coordinate family before Milestones 10.3–10.4 can assign every production coordinate to the finite-terrain stationary, internal-wave, or topographic-wave projector.

The pre-Milestone-6.7 checkpoint is commit [`d5ab18f`](https://github.com/JeffreyEarly/topographic-forcing/tree/d5ab18f) on the [`terrain-energy-galerkin`](https://github.com/JeffreyEarly/topographic-forcing/tree/terrain-energy-galerkin) branch. The current branch contains the completed projected, tangent boundary-complete, finite-amplitude weak, physical-subspace, modal-ambient, and terrain-dressed stationary-coordinate oracles. Milestone 10.2.5 is documented but not implemented. The matching mathematics is maintained in `finite-terrain-projection-problem.tex` and `terrain-energy-galerkin.tex` in the `ape-apv-bottom-topography` literature repository.
...
The latest implemented public entry point is `auditTerrainDressedRobinStationaryAmbient`. Its measured outcome is `dressed-stationary-blocker`; the tangent stationary correction is admissible and improves selected residuals, but no trained Robin family passes the complete stationary gate. The optional content-addressed cache writes only beneath ignored `output/`, validates every signature before reuse, and is disabled without disk writes by default. That result routes the roadmap to Milestone 10.2.5 rather than bottom/topographic-wave classification.

[Milestone 10.2.5](milestones.md#milestone-1025-global-tangent-scalar-stationary-compression-oracle) is the next planned increment. It will first factor the polynomial stationary oracle into zero-bottom-trace and lifted tangent-trace sectors, then independently test mixed-boundary modal compression without inserting oracle vectors into the candidate basis. Only `global-tangent-acceleration` authorizes Milestone 10.3.
...
The proposed global tangent-scalar factorization and its future public audit are specified in [Milestone 10.2.5](milestones.md#milestone-1025-global-tangent-scalar-stationary-compression-oracle). No `auditGlobalTangentStationaryCompression` implementation exists yet; this checkpoint intentionally stops before MATLAB work.
