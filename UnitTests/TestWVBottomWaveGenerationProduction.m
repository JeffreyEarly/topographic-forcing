classdef TestWVBottomWaveGenerationProduction < matlab.unittest.TestCase
    ...
    methods (Test)
        function variableStratificationMatchesOracleAndIdentities(testCase)
            ...
            for shouldAntialias = [false true]
                wvt = TestWVBottomWaveGenerationProduction.createTransform([8 6 5],shouldAntialias);
                ...
                for t = [0 813]
                    ...
                    testCase.verifyLessThanOrEqual(TestWVBottomWaveGenerationProduction.relativeError(Fp,directFp),1e-10)
                    testCase.verifyLessThanOrEqual(TestWVBottomWaveGenerationProduction.relativeError(Fm,directFm),1e-10)
                ...
                wvt.A0(:) = 0;
                diagnostics = TestWVBottomWaveGenerationProduction.sourceDiagnostics(wvt,forcing);
        ...
        function resolutionConversionRebuildsEquivalentForcing(testCase)
            ...
            testCase.verifyLessThanOrEqual(TestWVBottomWaveGenerationProduction.relativeError(convertedFp,expectedFp),1e-12)
            testCase.verifyLessThanOrEqual(TestWVBottomWaveGenerationProduction.relativeError(convertedFm,expectedFm),1e-12)
            ...
            highForcing = WVBottomWaveGenerationForcing(targetTransform,topographicHeight=highTerrain,barotropicVelocityAmplitude=forcing.barotropicVelocityAmplitude);
            ...
            testCase.verifyError(@()forcing.forcingWithResolutionOfTransform(incompatibleTransform),"WVBottomWaveGenerationForcing:IncompatibleDomain")
        end

        function explicitAntialiasConversionPreservesForcing(testCase)
            wvt = TestWVBottomWaveGenerationProduction.createTransform([8 6 5],true);
            terrain = TestWVBottomWaveGenerationProduction.topography(wvt);
            forcing = WVBottomWaveGenerationForcing(wvt,topographicHeight=terrain,barotropicVelocityAmplitude=[0.05; -0.02],name="antialias terrain");
            ...
            testCase.verifyClass(explicitForcing,"WVBottomWaveGenerationForcing")
            testCase.verifyEqual(explicitForcing.topographicHeight,forcing.topographicHeight,AbsTol=1e-12)
            explicitTransform.t = 391;
            diagnostics = TestWVBottomWaveGenerationProduction.sourceDiagnostics(explicitTransform,explicitForcing);
        ...
        function persistenceMetadataAndTransformRoundTrip(testCase)
            ...
            testCase.verifyClass(restoredForcing,"WVBottomWaveGenerationForcing")
            ...
            testCase.verifyLessThanOrEqual(TestWVBottomWaveGenerationProduction.relativeError(restoredFp,originalFp),1e-12)
        ...
        function adaptiveModelRestartMatchesControl(testCase)
            ...
            cleanup = onCleanup(@()TestWVBottomWaveGenerationProduction.deleteFile(restartPath));
            [restartModel,checkpointTime,finalTime] = TestWVBottomWaveGenerationProduction.modelForRestart();
            [controlModel,~,~] = TestWVBottomWaveGenerationProduction.modelForRestart();
            ...
            testCase.verifyClass(resumedModel.wvt.forcingWithName("restart model terrain"),"WVBottomWaveGenerationForcing")
            clear cleanup
        end

        function goffExampleAndPerformanceBenchmark(testCase)
            ...
            result = GoffAbyssalHillWaveGenerationExample(resolution=[16 16 7],minimumWavelength=150e3,numberOfOutputTimes=9,relativeTolerance=1e-7,shouldMakeFigures=true);
            figureCleanup = onCleanup(@()close(result.figureHandles(isgraphics(result.figureHandles))));

            testCase.verifyLessThanOrEqual(result.powerAgreement,1e-10)
            testCase.verifyLessThanOrEqual(result.normalizedQGPV,1e-10)
            testCase.verifyEqual(result.balancedEnergy,zeros(size(result.balancedEnergy)))
            testCase.verifyLessThan(result.energyWorkError,0.02)
            testCase.verifyGreaterThan(result.waveEnergy(end),0)
            testCase.verifyTrue(all(isfinite(result.energyByModeAndWavenumber),"all"))
            testCase.verifyNumElements(result.figureHandles,1)
            testCase.verifyTrue(isgraphics(result.figureHandles,"figure"))

            benchmark = BottomWaveGenerationPerformanceBenchmark(resolutions=[8 8 5; 12 12 7; 16 16 9],numberOfApplications=2);
            testCase.verifyEqual(height(benchmark),3)
            testCase.verifyTrue(all(benchmark.ConstructionSeconds > 0))
            testCase.verifyTrue(all(benchmark.ApplicationSeconds > 0))
            testCase.verifyTrue(all(diff(benchmark.EstimatedStoredBytes) > 0))
    ...
    methods (Static, Access = private)
        ...
        function [model,checkpointTime,finalTime] = modelForRestart()
            wvt = TestWVBottomWaveGenerationProduction.createTransform([8 6 5],false);
            forcing = WVBottomWaveGenerationForcing(wvt,topographicHeight=TestWVBottomWaveGenerationProduction.topography(wvt),barotropicVelocityAmplitude=[0.05; -0.01],rampDuration=100,startTime=0,name="restart model terrain");
            wvt.removeAllForcing();
            wvt.addForcing(forcing);
            model = WVModel(wvt);
